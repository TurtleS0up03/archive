...
ajax_status.onreadystatechange = function() {
  if(ajax_status.readyState == 4 && ajax_status.status == 200) {
      ...
      col=(col<<16)|(col>>16);
    writeTempMsgLines("responseText== "+ajax_status.responseText,col,5000);
...
function send_cmd (cmd) {
    ...
    writeTempMsgLines("send_cmd="+cmd,"red",10000);
...
function writeTempMsgLines(str,color,time) {
    
    var testElements = document.getElementsByClassName('TempMsgLines');
    var testDivs = Array.prototype.filter.call(testElements, function(element,index,array){ return element.innerHTML == '';});
    var linenum=0;
    var num=0;
    if (testDivs[linenum]!=undefined)
    {
	var temp=document.getElementById(testDivs[linenum].id).innerHTML="<p><b><font size=\"3\" color=\""+color+"\">"+str+"</font></b></p>";
	setTimeout(function(id){document.getElementById(id).innerHTML=""},time,testDivs[linenum].id); 
	delete testDivs[linenum];linenum++;
    }
